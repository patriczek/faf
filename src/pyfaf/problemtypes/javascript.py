...
__all__ = ["JavascriptProblem"]


class JavascriptProblem(ProblemType):
    name = "Javascript"
    nice_name = "Unhandled Javascript exception"

    checker = DictChecker({
        # no need to check type twice, the toplevel checker already did it
        # "type": StringChecker(allowed=[JavascriptProblem.name]),
    ...
    def __init__(self, *args, **kwargs):
        super(JavascriptProblem, self).__init__()

        hashkeys = ["processing.Javascripthashframes", "processing.hashframes"]
        self.load_config_to_self("hashframes", hashkeys, 16, callback=int)

        cmpkeys = ["processing.Javascriptcmpframes", "processing.cmpframes",
                   "processing.clusterframes"]
        self.load_config_to_self("cmpframes", cmpkeys, 16, callback=int)

        cutkeys = ["processing.Javascriptcutthreshold", "processing.cutthreshold"]
        self.load_config_to_self("cutthreshold", cutkeys, 0.3, callback=float)

        normkeys = ["processing.Javascriptnormalize", "processing.normalize"]
        self.load_config_to_self("normalize", normkeys, True, callback=str2bool)

        skipkeys = ["retrace.Javascriptskipsource", "retrace.skipsource"]
    ...
    def validate_ureport(self, ureport):
        JavascriptProblem.checker.check(ureport)
    ...
    def hash_ureport(self, ureport):
        ...
        for i, frame in enumerate(ureport["stacktrace"]):
            # Instance of 'JavascriptProblem' has no 'hashframes' member
    ...
    def save_ureport_post_flush(self):
        self.log_debug("save_ureport_post_flush is not required for Javascript")

    def _get_ssources_for_retrace_query(self, db):
        return None

    def find_packages_for_ssource(self, db, db_ssource):
        self.log_info("Retracing is not required for Javascript exceptions")
    ...
    def compare_many(self, db_reports):
        ...
        for db_report in db_reports:
            ...
            if report is None:
                self.log_debug("Unable to build satyr.JavascriptStacktrace")
                continue
